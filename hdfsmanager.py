...
class HDFSContentsManager(ContentsManager, HDFSManagerMixin):
    ...
    hdfs = Instance(HadoopFileSystem, config=True)

    @default('hdfs')
    def _default_hdfs(self):
        return HadoopFileSystem(host=self.hdfs_namenode_host, port=self.hdfs_namenode_port, user=self.hdfs_user, driver='libhdfs')
